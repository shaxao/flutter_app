...
class FlutterWebReminderManager {
  ...
  handlePushWakeUp(payload) {
    ...
    // 优先使用锁屏语音服务
    if (window.lockscreenVoiceService) {
      window.lockscreenVoiceService.speak(`提醒事项：${payload.body}`).then(() => {
        console.log('[ReminderManager] Lockscreen voice playback completed');
      }).catch(error => {
        console.error('[ReminderManager] Lockscreen voice error:', error);
        this.fallbackVoicePlayback(payload.body);
      });
    } else if (window.backgroundVoiceService) {
      // 备选：后台语音服务
  ...
  fallbackVoicePlayback(content) {
    // 备选语音播放方案
    console.log('[ReminderManager] Using fallback voice playback');
    ...
    if (window.flutterVoiceService) {
      ...
      console.error('[ReminderManager] No voice playback method available');
    }
  }
