...
class VoiceService {
  ...
  private keepAliveRetryCount = 0
  private readonly MAX_KEEP_ALIVE_RETRIES = 10

  // Platform detection
  private readonly isIOS = typeof navigator !== 'undefined' && /iPad|iPhone|iPod/.test(navigator.userAgent)
  private readonly isAndroid = typeof navigator !== 'undefined' && /Android/.test(navigator.userAgent)
  private readonly supportsPushManager = typeof window !== 'undefined' && 'PushManager' in window
  private readonly supportsMediaSession = typeof navigator !== 'undefined' && 'mediaSession' in navigator
  ...
  unlock() {
    ...
    console.log('Voice Service unlocked & Keep-alive started')
  }

  /**
   * Checks if the page is currently in background/hidden mode
   * @returns true if page is hidden (lockscreen or background)
   */
   ...
   */
  async preloadAudio(text: string, config?: TTSConfig): Promise<string | null> {
    if (!text) return null
   ...
   * @param text - The reminder text
   * @returns The cached audio entry or null
   */
  getCachedAudio(text: string) {
  ...
  async speak(text: string, config?: TTSConfig, retryCount = 0): Promise<void> {
    if (!text) return
    ...
    if (key) {
      try {
        ...
        this.logError('VOICE_PLAYBACK_FAILED', e as Error, {
          isBackground,
  ...
  private speakLocal(text: string): Promise<boolean> {
    return new Promise((resolve) => {
      ...
      // Background timeout: if no start event within 1s, consider it blocked
      const timeout = setTimeout(() => {
        console.warn('Local TTS timed out (likely background blocked)')
        this.synth?.cancel()
        resolve(false)
      }, 2000)
   ...
   */
  private async generateOpenAIAudio(text: string, baseUrl: string, key: string, config?: TTSConfig): Promise<Blob> {
  ...
  private async speakOpenAI(text: string, baseUrl: string, key: string, config?: TTSConfig) {
    // Check cache first
